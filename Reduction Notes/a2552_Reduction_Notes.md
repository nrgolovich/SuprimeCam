...
Range from SUPA00329640 to SUPA00329679 for July 18 2004
Range from SUPA00330870 to SUPA00330899 for July 19 2004

```
ln -s ../../rawdata/Abell2552/SUPA003296[4-7]?.fits .
ln -s ../../rawdata/Abell2552/SUPA003308[7-9]?.fits .
```
...
The flat field images were created using the procedure outlined in General Archival Data Process.md.
```
...
# V (2004jul18)
Note that there were V-band observations take on 2004jul18 (12 minutes total).
...
### Link to the raw data
These files range from SUPA00329390 to SUPA00329449. 2004jul18

```
ln -s ../../rawdata/Abell2552/SUPA0032939?.fits .
ln -s ../../rawdata/Abell2552/SUPA003294[0-4]?.fits .
```
...
```
python $gitdir/SuprimeCam/python/make_wht_for_swarp_2.py ../A115_V.fits 2.
```
...
# Fixing bad background in i

There is a strange halo around the stars in the i-band exposures and stack so I am experimenting with what the best SExtractor configuration is.

```
python $gitdir/SuprimeCam/python/run_sext_final.py swarp_wmean.fits -c sext_scam_final_a115.config
```

#### look at the images

```
ds9 -tile swarp_wmean_minusback.fits -scale log -scale limits 0 100 -zoom to fit swarp_wmean_background.fits -scale log -scale mode minmax -zoom to fit
```
