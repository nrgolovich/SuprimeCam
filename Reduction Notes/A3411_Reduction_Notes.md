...
1. Try turning off pattern-matching (MATCH N) in the scamp_scam.config file.

	```
	scamp object*cat -c scamp_scam.config -MATCH N
	```

2. Edit out any objects in the good_frames_pass1.txt file that had redlines in the scamp output. Then running through the following steps up to and through running SExtractor on the second swarp coadd, use this catalog as ASTREFCAT_NAME input in the scamp.config file, and repeat all of the following steps a second time.
