...
```
fieldcenter=137.40708,51.54667
```
...
```
ln -s ../../rawdata/2014feb25/SUPA014402[7-9]?.fits .
ln -s ../../rawdata/2014feb25/SUPA014403[0-4]?.fits .
```
...
A few failed on the first pass:
object025_sophie.c A1  P1  -1.29 deg  0.1986"   1.05     +27."    +52."   1.63
object029_san.cat  A1  P1  +0.18 deg  0.2018"  0.701     -5.1"    +14."   1.85
Note that more actually failed the first time but I accidentally ran the g ban files through first and these are the two that failed when matching to the g stack. so I will try a second pass after building a r stack without these two frames.
...
2. Edit out any objects in the good_frames_pass1.txt file that had redlines in the scamp output. Then run through the following steps up to and through running SExtractor on the second swarp coadd, use this catalog as ASTREFCAT_NAME input in the scamp.config file, and repeat all of the following steps a second time.
	...
	*Remove any objects with redlines from the good_frames_pass2.txt file.*
	all had astrometric solution on the second pass
...
```
ln -s ../../rawdata/2014feb25/SUPA014405[0-3]?.fits .
```
